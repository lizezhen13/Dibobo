...
- 用户点击“重新筛选”后才按当前缓存新鲜度重新获取数据并确定集合。

### 6.4.9 按需检索与指标缓存要求

红利雷达不再要求预先构建完整全市场指标快照。用户提交条件后创建后台搜索任务，接口立即返回 `search_id`，页面轮询任务阶段与进度。

搜索流程至少包括：

1. 读取或刷新 A 股代码池；
2. 批量刷新行情并排除 ST、退市整理和无有效行情标的；
3. 根据筛选条件和排序字段构建指标依赖计划；未参与筛选或排序的 PB、ROE、分红指标不主动刷新；
4. 仅在需要时读取或批量刷新 PB，并先执行 PB 条件过滤；
5. 仅在需要时对剩余候选集刷新原始分红事件，计算搜索时点股息率和连续分红年数并再次过滤；
6. 仅在需要时对再次缩小的候选集刷新 ROE，并完成最终过滤；
7. 形成冻结结果，搜索结果、指标值和排序上下文保存 24 小时，保证分页稳定；
8. 切换排序视为一次新的搜索，以便补齐新排序字段依赖的指标；
9. 单只股票指标失败应降级为缺失或旧缓存，不使整次搜索失败。

缓存新鲜度建议：

- 标的代码表：24 小时；
- 行情：搜索时批量刷新，结果页交易时段 5 秒刷新当前页；
- PB：交易中默认 5 分钟；
- ROE：默认 24 小时或发现新报告期后刷新；
- 原始分红事件：默认 24 小时；
- 获取失败：短期负缓存后重试；
- 股息率、连续分红年数和数据完整性：搜索时计算，不作为长期全局缓存。

任务调度和上游保护要求：

- 同一用户、数据源和标准化筛选条件产生搜索指纹；短时间内重复提交应复用排队中、执行中或刚完成的任务；
- 同一数据源同一时刻只执行一个雷达搜索，避免多个用户重复刷新相同的全市场基础数据；
- 对上游请求实施跨任务全局 QPS 限流；同一能力连续出现系统性错误时短时熔断，保留旧缓存并停止继续制造无效请求；
- Worker 进程复用持久事件循环，排队或执行超时的孤儿任务应自动回收为失败状态；
- 记录任务耗时、候选集逐层数量、上游错误码和请求标识，便于定位性能瓶颈。

首次搜索或候选集较大时可能耗时较长。页面应展示等待、代码池、行情、估值、财务与分红、最终筛选、完成和失败阶段；任务完成前不得把半成品当作最终结果返回。
...
### 8.6 `radar_indicator_cache`

| 字段 | 类型 | 说明 |
|---|---|---|
| `data_source_id` | FK | 对应用户数据源 |
| `thscode` | varchar | 股票代码 |
| `ticker` / `name` / `exchange` | varchar | 标的基础信息 |
| `latest` / `change_percent` | decimal/null | 最近成功行情 |
| `pb_mrq` | decimal/null | 最近成功 PB |
| `roe_weighted` / `roe_report_period` | decimal/varchar/null | 最近成功 ROE |
| `dividend_events` | jsonb | 原始现金分红事件 |
| `*_status` | varchar | available、not_available、not_fetched、fetch_failed、stale |
| `*_fetched_at` | timestamptz/null | 各类数据最近获取时间 |
| `last_error` | varchar/null | 最近安全错误摘要 |

建议主键：`data_source_id + thscode`。

### 8.7 `radar_search_jobs`

| 字段 | 类型 | 说明 |
|---|---|---|
| `id` | UUID | `search_id` |
| `user_id` / `data_source_id` | FK | 用户与数据源归属 |
| `state` | varchar | queued、running、ready、failed |
| `stage` / `stage_message` | varchar | 当前检索阶段与说明 |
| `filters` | jsonb | 本次筛选条件 |
| `processed_count` / `candidate_count` | integer | 任务进度 |
| `total_results` / `incomplete_results` / `stale_results` | integer | 结果统计 |
| `created_at` / `started_at` / `completed_at` | timestamptz/null | 生命周期 |
| `expires_at` | timestamptz | 结果过期时间 |
| `error_summary` | text/null | 失败摘要，不含密钥 |

### 8.8 `radar_search_results`
...
| `data_incomplete` / `data_stale` | boolean | 三值缺失和旧缓存标识 |
| `stale_fields` | jsonb | 使用旧缓存的字段集合 |

建议主键：`search_id + thscode`。
...
- `GET /api/radar/search/{search_id}/status`
- `GET /api/radar/results?search_id=...&page=...&sort_by=...`
- `GET /api/radar/quotes?search_id=...&page=...`

筛选请求使用 POST，以结构化 JSON 传递区间和初始排序，返回后台任务 `search_id`；任务完成后通过冻结结果接口执行稳定分页与排序。
...
- 缓存完全命中的雷达搜索目标 2 秒内完成；首次搜索或缓存过期时展示后台进度；
- 持仓和日记 CRUD 目标 1 秒内返回；
- 用户操作期间不得因后台雷达搜索任务阻塞持仓和日记功能。
...
| 主数据库 | PostgreSQL 18.x | 保存用户、会话、数据源配置、持仓、日记、雷达指标缓存、搜索任务和冻结结果 |
| 后台任务 | Celery 5.x | 执行按需雷达搜索、候选指标刷新和失败降级；不得承载前端每 5 秒行情轮询 |
...
- API 与后台任务共用同一后端代码与容器镜像，但必须以独立进程和独立 Compose 服务运行；
- 交易时段 5 秒行情刷新由前端请求后端、后端合并请求并读取或更新短期缓存完成，不通过 Celery 为每次页面刷新创建任务；
- 雷达候选集的财务、估值和分红刷新必须在后台 Worker 中执行，不得占用 API 请求进程执行长任务；
...
| `worker` | Celery Worker，执行按需雷达搜索、指标刷新和失败降级 |
| `postgres` | 用户业务数据、雷达指标缓存、搜索任务和冻结结果持久化 |
| `valkey` | 短期行情缓存、限流状态和 Celery 消息代理 |

`api` 和 `worker` 应从同一个后端应用镜像启动不同命令，避免形成两套代码；`worker` 默认并发数应适配小型服务器资源，并可通过环境变量调整。
...
删除或重建应用容器不得删除数据库卷。Valkey 不得作为指标缓存、搜索任务或冻结结果的唯一存储；系统重启后必须能从 PostgreSQL 和第三方数据源恢复可重建数据。
...
- 可配置行情目标刷新周期，默认 5 秒；
- 可配置第三方请求超时与并发限制；
- 可配置 Worker 并发数、任务超时、各类指标缓存 TTL 和失败重试间隔；
- 可选首次初始化用户信息。
...
- 数据库迁移应在后端正式对外服务前完成；
- 初始化用户命令应有清晰说明；
- `api`、`worker`、`postgres` 和 `valkey` 均应提供健康检查或等价的就绪判断；
...
16. 重新筛选后按当前缓存新鲜度创建新的后台搜索任务；
17. 首次搜索展示阶段进度，完成前不返回半成品结果；
...
1. `web`、`api`、`worker`、`postgres` 和 `valkey` 可通过 Docker Compose 启动；
2. 前端、API、后台任务、数据库、缓存和反向代理均运行在容器中；
3. 容器重建后用户数据不丢失；
4. 提供健康检查、环境变量示例、初始化账号命令和备份说明；
5. 生产密钥不写入镜像和代码仓库；
6. 单套部署支持 1～2 名用户正常使用且后台搜索不阻塞主要功能；
7. 前后端生产依赖由锁文件固定，实际构建结果符合第 12.1 节技术栈基线；
8. 雷达按需搜索由独立 Worker 执行，页面 5 秒行情刷新不创建 Celery 任务。
...
- [ ] 技术栈、模块化单体边界及 API/Worker 进程拆分符合第 12 节要求；
