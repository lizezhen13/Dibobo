...
- AKShare 作为系统级共享备用源；
- AKShare 使用 `stock_zh_a_spot_em()` 获取全沪深京 A 股快照；
- AKShare 精确锁定版本并运行在后台任务环境中。
...
flowchart LR
    ...
    COMPOSITE --> API
    WORKER["market_data Worker"] --> AK
    SCHEDULER["调度器"] --> WORKER
    AK --> PROVIDER_CACHE["全局 AKShare 快照"]
...
| Provider Cache | 保存单个 Provider 的标准化新鲜快照和最后成功快照 |
...
- 扶摇不支持总市值；
- AKShare 总市值路由已启用且健康；
- 则系统的 `total_market_cap` 有效能力为 `supported`；
- 红利雷达总市值筛选保持可用。

若 Provider 静态支持能力但当前快照暂时不可用，控件不必立即禁用；搜索可以继续执行，并按现有三值逻辑标记缺失标的。只有完整路由链均不支持该能力时，才将控件禁用。
...
## 10. AKShare 采集、缓存与任务调度

### 10.1 接口范围

首期使用：

- `stock_zh_a_spot_em()`：全沪深京 A 股实时快照、总市值、动态 PE、PB、成交量额等；
- 官方字段参考：[AKShare 股票数据文档](https://akshare.akfamily.xyz/data/stock/stock.html)。
...
AKShare 为同步 DataFrame 抓取库，不得在 FastAPI 事件循环或浏览器业务请求中直接执行全市场抓取。

系统必须：

1. 使用独立 Celery `market_data` 队列；
2. 由明确的调度器服务触发刷新；
3. 一次调用获取全市场，一次标准化后写入 Valkey；
4. 所有用户共享同一份 AKShare 快照；
5. 不按用户、持仓或股票逐只重复抓取；
6. 使用全局 single-flight 分布式锁；
7. 新快照完整生成并通过质量门禁后原子发布；
8. 新快照失败不得破坏最后成功快照；
9. API 请求遇到备用快照未就绪时，不得同步抓取或无限等待。

### 10.3 默认刷新计划

| 市场状态 | 默认刷新频率 |
|---|---:|
| 交易中 | 每 60 秒一次，附带 0～10 秒随机抖动 |
| 午间休市 | 每 5 分钟一次 |
| 已收盘 | 15:10 刷新一次；失败时在 15:20、15:30 最多重试两次；成功后停止至下次预热 |
| 周末、法定休市 | 不执行计划刷新；保留最近有效收盘快照 |
| Worker 或调度器启动 | 立即执行一次预热 |

在上游可用的前提下，Worker 和调度器启动后 90 秒内应产生首份有效快照。

“同一刷新窗口”定义为同一 `capability + scheduled_at` 调度实例。该实例的 single-flight 锁默认有效 90 秒；锁持有期间，浏览器请求、其他用户请求和重复调度均不得创建第二次 AKShare 上游抓取。

### 10.4 缓存层级

#### Provider 快照缓存

保存 AKShare 的标准化全市场快照：
...
策略、Provider 顺序、能力开关、映射版本或质量规则变化后，必须生成新的 fingerprint，不得命中旧组合结果。`quality_profile` 变化后，旧 Provider 快照必须重新校验或通过新缓存键隔离。

### 10.5 新鲜度与硬过期

| 数据组 | 交易时段新鲜度 | 最后成功数据规则 |
|---|---:|---|
| AKShare 核心行情 | 不超过 90 秒 | 90 秒至 5 分钟仅可作为 stale 展示；超过 5 分钟不得参与持仓市值、盈亏或新筛选计算 |
| 总市值 | 不超过 5 分钟 | 最近有效交易时段收盘值可在非交易时段使用；单只标的连续 3 个交易日无新值后停止用于新计算 |
| 动态 PE、PB | 不超过 5 分钟 | 正式启用前另行确认非交易时段口径 |
| Provider 最后成功快照 | 物理保留至少 10 个自然日 | 是否可参与计算仍受市场状态和各能力硬过期规则约束 |
| 现有组合行情缓存 | 按现有 24 小时缓存保留 | 仅用于现有行情降级，不替代 Provider 快照的跨周末保留 |
...
每日质量门禁使用“当日 A 股基准股票池”作为覆盖率分母。该股票池取用户主源或最近成功主源目录在 `Asia/Shanghai` 当日 09:15 后的第一份有效 `instrument_list`，范围为沪、深、北且 `asset_type=a_share` 的 active 标的：
...
- Valkey 读取失败时可继续使用主源，但不得同步触发 AKShare 全市场抓取；
...
1. AKShare 总市值快照健康且 `total_market_cap` 路由处于 `serve` 时，有效能力为 supported；
2. 总市值筛选控件恢复可用；
3. 用户输入和页面展示继续使用亿元；
4. 内部标准值继续使用人民币元；
5. 最小值和最大值边界均包含；
...
性能验收使用以下基线：API/Worker 各分配至少 2 vCPU、4 GB 内存，PostgreSQL 与 Valkey 位于同一部署网络；标准化数据集为 7,000 条标的；使用 10 个并发虚拟用户持续 5 分钟；上游抓取不计入缓存命中测试并使用固定 fixture 或已发布快照。

在该基线下：

- 组合缓存命中请求 P95 不超过 500 ms、P99 不超过 1 秒；
- AKShare 新鲜快照命中后，网关合并额外 P95 耗时不超过 100 ms；
- Web 请求不得等待同步全市场抓取；
- 单份全 A 股快照至少支持 7,000 条记录；
- 7,000 条快照的标准化、校验和原子发布必须在抓取完成后 30 秒内完成；
- 后台 AKShare 任务不得阻塞持仓 CRUD、日记或扶摇行情请求；
- 多用户请求不得重复触发相同 AKShare 抓取。
...
### 20.2 阶段 1：AKShare 影子采集

- 增加 AKShare Adapter、独立队列和调度器；
- 锁定依赖版本；
- 抓取全 A 股快照并完成标准化；
- 只写影子快照和比较指标；
- 连续运行至少 3 个完整交易日；
...
| BR-17 | AKShare 不得在 FastAPI 业务请求内同步抓取 |
| BR-18 | AKShare 全市场快照按部署全局共享，不能按用户重复抓取 |
...
- [ ] AKShare 使用一次全市场快照提供 A 股总市值，不按用户或股票逐只抓取；
...
### 22.4 AKShare Worker 与缓存

- [ ] AKShare 全市场抓取不在 FastAPI 业务请求内执行；
- [ ] 多用户同时请求相同数据时，同一刷新窗口全局最多一次 AKShare 上游抓取；
- [ ] Worker 和调度器启动且上游可用时，90 秒内生成有效快照；
- [ ] 新快照通过完整质量门禁后原子发布；
- [ ] 必需列缺失、覆盖率不足或 schema 不兼容时，新快照不发布；
- [ ] 新快照失败不会破坏最后成功快照；
- [ ] Valkey 写入失败不会暴露半份快照；
...
- [ ] 总市值有效覆盖率和代码覆盖率达到第 11.3 节门槛；
- [ ] 总市值正式启用前完成至少 3 个完整交易日影子验证；
...
- [ ] 7,000 条快照转换和并发缓存命中性能达到第 19 节目标。
...
| 备用源状态 | 新鲜、部分覆盖、空 DataFrame、列改名、重复代码、解析异常、超时、限流、质量失败 |
| 缓存 | 命中、未命中、预热中、stale 未过硬期限、已过硬期限、Valkey 断开、并发击穿 |
...
| 多用户 | 共享 AKShare 快照、不同扶摇 Key 隔离、并发请求、用户切换主源 |
...
| 性能 | 7,000 条快照、并发用户、缓存命中 P95、内存占用、队列积压 |
| 契约 | 固定 DataFrame 样本、真实影子请求、必需列、schema fingerprint、代码和单位映射 |
...
| 备用源增加请求量 | 限流或封禁 | 后台全局快照、single-flight、缓存、固定频率、退避 |
