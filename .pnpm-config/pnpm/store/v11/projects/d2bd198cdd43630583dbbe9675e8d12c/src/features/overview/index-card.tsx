    ...
    <article className="relative min-w-0 border-b border-border/80 px-3.5 pt-2.5 pb-1.5 last:border-b-0 min-[900px]:border-r min-[900px]:[&:nth-child(even)]:border-r-0 min-[900px]:[&:nth-last-child(-n+2)]:border-b-0 min-[1400px]:border-b-0 min-[1400px]:border-r min-[1400px]:[&:nth-child(even)]:border-r min-[1400px]:last:border-r-0">
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          <div className="flex items-center gap-2">
            <span className="font-mono text-[11px] text-muted-foreground/50">{String(ordinal).padStart(2, "0")}</span>
            <h3 className="truncate text-[15px] font-semibold tracking-normal text-foreground">{data.name}</h3>
            <span className="shrink-0 font-mono text-[11px] tracking-normal text-muted-foreground/55">{data.thscode}</span>
          </div>
        </div>
      ...
      <div className="mt-2.5 grid grid-cols-2 gap-x-5 gap-y-1.5 border-t border-border/70 pt-2">
        ...
        <div className="flex min-w-0 items-baseline justify-between gap-2">
          ...
          <span className="truncate font-mono text-xs tracking-normal text-muted-foreground">{formatMoney(data.turnover)}</span>
        </div>
        <div className="flex min-w-0 items-baseline justify-between gap-2">
          <span className="shrink-0 text-[11px] text-muted-foreground/60">最低</span>
          <span className="truncate font-mono text-xs tracking-normal text-foreground/80">{formatPoint(data.low, { group: false })}</span>
        </div>
        <div className="flex min-w-0 items-baseline justify-between gap-2">
          <span className="shrink-0 text-[11px] text-muted-foreground/60">最高</span>
          <span className="truncate font-mono text-xs tracking-normal text-foreground/80">{formatPoint(data.high, { group: false })}</span>
        </div>
      </div>
    </article>
  );
