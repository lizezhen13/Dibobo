...
export function PortfolioStockDetailPage() {
  const { portfolioId: routePortfolioId, ticker: routeTicker } = useParams<{ portfolioId: string; ticker: string }>();
  ...
  const returnTo = portfolioId ? `/portfolios?portfolio=${encodeURIComponent(portfolioId)}` : "/portfolios";

  if (!stockPageUrl) {
    return <InvalidTickerState returnTo={returnTo} />;
  }

  return (
    <PageContainer size="fluid" className="flex h-[calc(100vh-68px)] min-h-0 flex-col overflow-hidden bg-background">
      <h1 className="sr-only">{ticker} 投资组合持仓详情</h1>
      <div className="flex h-14 shrink-0 items-center justify-between border-b border-border bg-background px-5">
        <div className="hidden items-center gap-2 text-[0.7rem] text-muted-foreground/65 sm:flex">
          ...
          <span className="font-mono tracking-[0.14em]">PORTFOLIO DETAIL · {ticker}</span>
        </div>
...
function InvalidTickerState({ returnTo }: { returnTo: string }) {
  return (
    <PageContainer size="wide">
      <div className="rounded-xl border border-border bg-card px-6 py-16 text-center shadow-raised">
        ...
        <p className="mt-5 font-mono text-[0.64rem] tracking-[0.16em] text-primary/75">PORTFOLIO / INVALID CODE</p>
