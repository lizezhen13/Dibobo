  ...
  return (
    <span className="flex items-center gap-1.5">
      {event.scope_tags.map((tag) => (
        <span
          key={tag}
          className="rounded-md border border-primary/20 bg-primary/10 px-1.5 py-0.5 font-mono text-caption uppercase tracking-[0.12em] text-primary-text"
        >
          {tag === "watchlist" ? "自选" : "持仓"}
        </span>
      ))}
...
export function EventCard({ event, onClick }: { event: CalendarEvent; onClick: () => void }) {
  ...
  return (
    ...
    >
      ...
      <span className="min-w-0 flex-1">
        <span className="flex items-start justify-between gap-3">
          <span className="min-w-0">
            <span className="block truncate text-table font-semibold tracking-tight text-foreground sm:text-body">
              {isCompanyEvent && event.security_name ? event.security_name : event.title}
            </span>
            <span className="mt-0.5 block truncate text-table text-muted-foreground">
              {isCompanyEvent && event.security_name ? `${event.symbol ?? ""} · ${event.title}` : eventSubtitle(event)}
            </span>
          </span>
          <span className="shrink-0 pt-0.5 font-mono text-caption text-muted-foreground">{timeLabel(event)}</span>
        </span>
        <span className="mt-2 flex min-h-4 flex-wrap items-center gap-x-2.5 gap-y-1 text-caption text-muted-foreground">
      ...
      <ChevronRight size={15} className="mt-3 shrink-0 text-subtle transition group-hover:translate-x-0.5 group-hover:text-primary-text" />
    </button>
  );
}
...
function DetailRow({ label, value }: { label: string; value: unknown }) {
  ...
  return (
    <div className="grid grid-cols-[88px_minmax(0,1fr)] gap-4 border-b border-border/60 py-3 last:border-0">
      ...
      <dd className="break-words text-right text-table text-foreground">{String(value)}</dd>
    </div>
  );
}
      ...
      <SheetContent side="right" showClose={false} aria-describedby="calendar-event-drawer-description" className="calendar-drawer">
        <div className="flex items-start justify-between border-b border-border px-6 py-5">
          <div className="flex items-start gap-3">
            ...
            <div>
              <p className={cn("font-mono text-caption tracking-[0.18em]", meta.tone)}>{meta.eyebrow}</p>
              <SheetTitle id="calendar-event-drawer-title" className="mt-1 text-title font-semibold">
                {event.category !== "macro" && event.category !== "closed" && event.security_name ? event.security_name : event.title}
              </SheetTitle>
              <SheetDescription id="calendar-event-drawer-description" className="mt-1 text-body-sm text-muted-foreground">
                {eventSubtitle(event)}
              </SheetDescription>
            </div>
          </div>
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5">
          <div className="rounded-2xl border border-border bg-background/45 p-4">
            <div className="flex items-center justify-between gap-4">
              ...
              <span className="font-mono text-body-sm text-foreground">{timeLabel(event)}</span>
            </div>
            <p className="mt-3 text-body leading-7 text-foreground">{event.content || event.title}</p>
          </div>
          ...
          {event.scope_tags.length > 0 ? (
            <div className="mt-5 rounded-2xl border border-primary/15 bg-primary/[0.06] p-4">
              <div className="flex items-center gap-2 text-body-sm text-primary-text">
                <Tag size={14} /> 关注范围
              </div>
              <div className="mt-3 flex gap-2">
                {event.scope_tags.map((tag) => (
                  <Badge key={tag} variant="neutral" className="border-primary/20 bg-primary/10 text-primary-text">
                    {tag === "watchlist" ? "自选" : "持仓"}
                  </Badge>
                ))}
              </div>
        ...
        <div className="border-t border-border px-6 py-4 text-caption text-muted-foreground">
