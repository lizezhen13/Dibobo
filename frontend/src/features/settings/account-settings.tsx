...
export function AccountSettings() {
  ...
  const errorMessage = mutation.error instanceof ApiError ? mutation.error.message : mutation.error ? "密码修改失败，请稍后重试" : null;
      ...
      <Card className="overflow-hidden">
        <form onSubmit={onSubmit} className="p-7" noValidate>
          <div className="grid grid-cols-2 gap-x-6 gap-y-5">
            <FormField label="原密码" error={form.formState.errors.current_password?.message} className="col-span-2 max-w-[calc(50%-12px)]">
              <Input type="password" autoComplete="current-password" {...form.register("current_password")} />
            </FormField>
            <FormField label="新密码" error={form.formState.errors.new_password?.message}>
              <Input type="password" autoComplete="new-password" {...form.register("new_password")} />
            </FormField>
            <FormField label="确认新密码" error={form.formState.errors.confirm_password?.message}>
          ...
          {errorMessage && <InlineAlert className="mt-5">{errorMessage}</InlineAlert>}
