...
const groupedPointFormatter = new Intl.NumberFormat("zh-CN", {
  minimumFractionDigits: 2,
  maximumFractionDigits: 4,
  useGrouping: true,
});
const ungroupedPointFormatter = new Intl.NumberFormat("zh-CN", {
  minimumFractionDigits: 2,
  maximumFractionDigits: 4,
  useGrouping: false,
});
...
export function formatPoint(value: number | null, options?: { group?: boolean }): string {
  ...
  return ((options?.group ?? true) ? groupedPointFormatter : ungroupedPointFormatter).format(value);
}
